...
- **Workspace governance files sync via the whitelist repo rooted at `~/workspace`** (W-R26: private remote `workspace-meta`; tracks ONLY `CLAUDE.md` + `feedback-register.md`, everything else gitignored). After editing them: commit + push that repo; when resuming on another machine: `git -C ~/workspace pull` first. Per-host facts (`~/.claude/`) never enter it; project rules travel with their own repos.
