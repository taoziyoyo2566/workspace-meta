...
Gate 2 (lint/parse) is **not** a substitute for gate 3. "Done" requires all three clean in the **same iteration**. **Iteration cap = 3**: after 3 consecutive failures of the same gate, stop and surface — the root cause is deeper. Record gate outcomes in the changelog. If a gate genuinely can't run here (missing tool/env), record it as **blocked, never as passed** — and "can't run" must come from a **fresh probe, not memory** (W-R24): environment capability claims (tool present/absent, daemon up, creds) are dated snapshots; re-probe before they become load-bearing, then update the stale record in the same round. The persistent record is the shared per-host registry `~/workspace/.agents/env/<host>.yml` (refresh `make -C ~/workspace env-probe`; freshness gate `env-probe-check`; behavior rule `~/workspace/.agents/rules/environment-truth.md`) — **consult it before re-deriving probes ad hoc**, and note that host-bare tool absence (e.g. ansible in a project venv) is expected: each project's `.agents/env/README.md` carries its own venv-tool overrides.
...
## 5. Editing rules / config / docs  (W-R8, W-R9, W-R10, W-R14, W-R27)
...
- **Match a behavior rule's form to its failure type** (W-R27). Before writing a rule meant to shape how the agent acts under pressure (not a mechanical fact/path/version), classify the baseline failure and pick the matching form: *discipline-skip* (knows it, breaks it under pressure) → prohibition + rationalization table + red-flags; *wrong-shaped output* (complies but bloats / buries the verdict / restates the spec) → a positive recipe stating what the output IS, in order; *omitted element* → a REQUIRED slot in the template they already fill; *condition-dependent* → a conditional keyed to an observable predicate, not an unconditional rule with exemption clauses. **Prohibitions backfire on shape/omission failures** — don't default to "never/don't", and add no nuance/exemption clauses (they reopen the negotiation). The register's source-incident requirement already supplies the baseline-failure evidence; constructed pressure-scenarios + micro-tests are an optional escalation for high-stakes/disputed rules (register W-R27), not a standing gate. A rule that **recurs in a later real session** is the REFACTOR signal — change the form before amplifying the language. Full failure→form table + persuasion/bulletproofing toolkit: register W-R27.
